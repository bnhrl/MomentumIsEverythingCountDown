...
func _ready() -> void:
	...
	tween.tween_property($Title, "offset_transform_scale", Vector2.ZERO, 0.333).set_trans(Tween.TRANS_EXPO)
	tween.parallel().tween_property($Subtitle, "offset_transform_scale", Vector2.ZERO, 0.333).set_trans(Tween.TRANS_EXPO)
